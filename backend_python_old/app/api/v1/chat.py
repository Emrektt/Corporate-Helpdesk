...
def serialize_message(msg: ChatMessage) -> dict:
    return {
        ...
        "attachment_url": f"/api/v1/chat/messages/{msg.id}/download" if msg.attachment_url else None,
...
async def upload_chat_attachment(
    ...
    """Sohbete dosya/fotoğraf yükler ve bunu bir mesaj olarak ekler."""
    room = db.query(ChatRoom).filter(ChatRoom.id == room_id).first()
    if not room:
        raise HTTPException(status_code=404, detail="Oda bulunamadı")
    if room.status == ChatRoomStatus.CLOSED:
        raise HTTPException(status_code=400, detail="Kapalı odaya dosya gönderilemez")

    # Dosyayı kaydet
    ext = os.path.splitext(file.filename)[1] if file.filename else ""
    safe_name = f"chat_{room_id}_{uuid.uuid4().hex}{ext}"
    ...
    msg = ChatMessage(
        room_id=room_id,
        sender_id=current_user.id,
        content=file.filename or "Dosya",
        is_system=False,
        attachment_url=safe_name, # Sadece fiziksel adı tutuyoruz
        attachment_type=att_type,
        attachment_name=file.filename or "Dosya"
    )
    db.add(msg)
    db.commit()
    db.refresh(msg)

    # Websocket üzerinden odadaki herkese duyur
    await manager.broadcast_to_room(room_id, {
        "type": "message",
...
def download_chat_attachment(
    ...
    """Bir mesajdaki eki indirir."""
    msg = db.query(ChatMessage).filter(ChatMessage.id == message_id).first()
    if not msg or not msg.attachment_url:
        raise HTTPException(status_code=404, detail="Dosya bulunamadı")
    
    file_path = os.path.join(UPLOAD_DIR, msg.attachment_url)
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail="Dosya sunucuda bulunamadı")
        
    media_type = "image/jpeg" if msg.attachment_type == "image" else "application/octet-stream"
    return FileResponse(path=file_path, filename=msg.attachment_name, media_type=media_type)
