...
def get_tickets(
    ...
    is_standard_user = current_user.role not in [UserRole.ADMIN, UserRole.SUPPORT_AGENT]
    if is_standard_user or as_user:
        query = query.filter(Ticket.created_by_id == current_user.id)

    # Test Biletlerini Filtreleme
    env = os.getenv("ENVIRONMENT", "development").lower()
    # 1. Normal kullanıcılardan her zaman testleri gizle
    # 2. Production ortamında testleri HERKESTEN gizle (Adminler dahil)
    if is_standard_user or env == "production":
        query = query.filter(not_(Ticket.title.ilike('%test%')))
